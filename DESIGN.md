...
│  └──────────────────────────────────────┘      │
│                                              │
│  自訂 API（WAB Servlet, /appointment/）       │
│    ├── GET  /init          初始資料（一次載入）│
│    ├── GET  /events        預約查詢（按日期）  │
│    ├── POST /book          建立預約（原子操作）│
│    ├── PUT  /update        更新預約（群組同步）│
│    ├── POST /group-add     加入資源（衝突檢測）│
│    ├── DELETE /group-remove 移除資源           │
│    ├── GET  /bpartners     搜尋病患           │
│    └── POST /token         JWT 換發           │
│                                              │
│  iDempiere REST API（寫入操作的 fallback）     │
│    ├── S_ResourceAssignment  拖拉改時間       │
│    ├── C_Order / C_OrderLine 計費             │
│    └── processes/            DocAction        │
│                                              │
└──────────────────────────────────────────────┘
```
...
└── src/
    ...
    ├── api.ts                    ← API client（自訂 API + REST API）
    ├── components/
    │   ├── Calendar.tsx          ← FullCalendar wrapper + 事件分組渲染
    │   ├── Sidebar.tsx           ← 資源面板 + 狀態圖例 + 搜尋
    │   └── AppointmentDialog.tsx ← 新增/編輯 + 資源標籤編輯器
    └── hooks/
        └── useAppState.ts        ← 集中狀態管理 + 所有操作
```

### 核心設計：預約分組

DB 中每個資源有獨立的 `S_ResourceAssignment` 記錄，用 `Description` JSON 中的 `group_id` 關聯。SPA 在渲染前將 assignments 分組為 appointments：

```
assignments (from /appointment/events)
  → groupByAppointment()           // 用 group_id 分組
  → Appointment[]                  // 每個 appointment 有 resources[]
  → FullCalendar events            // 每個 appointment 顯示 1 個事件
```

行事曆上每個事件代表一個「預約」（可能包含多個資源），不是單一 assignment。
│  │  └── 呼叫 /api/v1/models/*          │      │
│  └──────────────────────────────────────┘      │
│                                              │
